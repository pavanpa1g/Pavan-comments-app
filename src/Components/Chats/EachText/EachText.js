...
const EachText = ({ item }) => {
  ...
  return (
    <div className={`flex mb-2 ${!isSendByMe ? 'ml-auto' : 'mr-auto'}  self-start max-w-[50%]`}>
      {!isSendByMe && (
        <img src={picture} alt="profile" className="w-[16px] h-[16px] rounded-[50%] mr-1" />
      )}
      <span className={`${sender._id === userSelector._id ? 'bg-[#BEE3F8]' : 'bg-[#B9F5D0]' }`}>
        {/* <h1>{}</h1> */}
